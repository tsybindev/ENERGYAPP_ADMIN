...
const AddCourse = ({ ...props }: ComponentProps) => {
	...
	const onSubmit: SubmitHandler<
		z.infer<typeof CourseAddSchema>
	> = async data => {
		const { title } = data

		const toastId = toast.loading('Создание лекции...')

		try {
			const token = Cookies.get('users_access_token')
			const response = await createLesson(token, title, props.module_id)

			toast.success('Лекция добавлена успешно!', {
				id: toastId,
				duration: 5000,
				richColors: true,
				action: (
			...
			await router.push(`/course/module/lesson/${response.item_id}`)
		} catch (e) {
	...
	return (
		<Sheet>
			...
			<SheetContent>
				<SheetHeader>
					<SheetTitle>Добавить лекцию</SheetTitle>
				</SheetHeader>
